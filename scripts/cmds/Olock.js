module.exports = {
  config: {
    ...
    category: "box chat",
    guide: {
      en: `lock nickname on/off → lock/unlock nicknames
lock groupname on/off → lock/unlock group name
    ...
    switch (type) {
      case "nickname": {
        if (action === "on") {
          ...
          return message.reply("✅ Sabhi members ke nicknames lock ho gaye (sirf bot admins change kar sakte hain).");
        }
        if (action === "off") {
          settings.lockNick = false;
          await threadsData.set(threadID, settings, "data.groupLock");
          return message.reply("❌ Nickname lock band kar diya gaya.");
        }
        break;
      }
      ...
      case "groupname": {
        if (action === "on") {
          ...
          await threadsData.set(threadID, settings, "data.groupLock");
          return message.reply("✅ Group name lock ho gaya (sirf bot admins change kar sakte hain).");
        }
        if (action === "off") {
          settings.lockName = false;
          await threadsData.set(threadID, settings, "data.groupLock");
          return message.reply("❌ Group name lock band kar diya gaya.");
        }
        break;
      }
...
- Group Name: ${settings.lockName ? "ON" : "OFF"}`
      ...
      default:
        return message.reply("❓ Use: lock [nickname|groupname|status] [on|off]");
    }
  },
